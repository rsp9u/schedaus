...
class Renderer:
    ...
    def render(self, data):
        ...
        self.dwg.attribs["width"] = f"{mw}px"
        self.dwg.attribs["height"] = f"{mh}px"
    ...
    def calendar_to_svg(self, calendar, data):
        ...
        while date <= calendar.end:
            if date.month != prev_month:
                ...
                objs.append({"z": 10, "o": self.dwg.line((x, 0), (x, mh), **line_common_opts)})
                prev_month = date.month
            objs.append({"z": 5, "o": self.dwg.text(date.strftime("%a")[0:2], (x+w/2, h+ph), **text_day_opts)})
            objs.append({"z": 5, "o": self.dwg.text(date.day, (x+w/2, h*2+ph), **text_day_opts)})
            objs.append({"z": 10, "o": self.dwg.line((x, h*3), (x, mh), **line_common_opts)})
            x += w
            date += timedelta(days=1)
        for closed in calendar.closed:
            offset_days = (closed - calendar.start).days
            xy = (w * offset_days, h)
            objs.append({"z": 0, "o": self.dwg.rect(xy, (w, mh-h), fill="#D0D0D0")})

        # Outer frame
        objs.append({"z": 10, "o": self.dwg.line((mw, 0), (mw, mh), **line_common_opts)})
        objs.append({"z": 10, "o": self.dwg.line((0, h*0), (mw, h*0), **line_common_opts)})
        objs.append({"z": 10, "o": self.dwg.line((0, h*3), (mw, h*3), **line_common_opts)})

        # Today's line
        x = w * (calendar.today - calendar.start).days
        objs.append({"z": 99, "o": self.dwg.line((x, 0), (x, mh), stroke="#FF0000", stroke_width="3.0")})
    ...
    def milestone_to_svg(self, calendar, milestone):
        ...
        t = self.dwg.text(milestone.text, (w+2, 2), **text_ms_opts)
    ...
    def group_to_svg(self, calendar, group):
        ...
        objs.append(self.dwg.line((0, h/5*2), (mw/2 - text_size/2, h/5*2), **line_opts))
        objs.append(self.dwg.line((0, h/5*3), (mw/2 - text_size/2, h/5*3), **line_opts))
        objs.append(self.dwg.text(group.text, (mw/2, h/2), **text_opts))
        objs.append(self.dwg.line((mw/2 + text_size/2, h/5*2), (mw, h/5*2), **line_opts))
        objs.append(self.dwg.line((mw/2 + text_size/2, h/5*3), (mw, h/5*3), **line_opts))
        return objs
